...
pub static HTML_ID_TEMPLATE_PATH: once_lazy<String> = once_lazy::new(|| {
    dotenv().ok();
    env::var("HTML_ID_TEMPLATE_PATH").expect("[ENV file read Error] 'HTML_ID_TEMPLATE_PATH' must be set")
});
...
pub static HTML_PW_TEMPLATE_PATH: once_lazy<String> = once_lazy::new(|| {
    dotenv().ok();
    env::var("HTML_PW_TEMPLATE_PATH").expect("[ENV file read Error] 'HTML_PW_TEMPLATE_PATH' must be set")
});
