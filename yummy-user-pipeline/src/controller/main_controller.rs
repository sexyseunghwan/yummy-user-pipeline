...
use crate::models::send_join_form::*;

use crate::utils_module::io_utils::*;

pub struct MainController<
    K: KafkaService + Send + Sync + 'static,
...
impl<K: KafkaService + Send + Sync + 'static, S: SmtpService + Send + Sync + 'static>
    MainController<K, S>
{
            ...
            match result {
                Ok(message) => {
                    let res = match topic {
                        ...
                        "yummy-join-hist" => self.user_join_hist(consumer, &message).await,
                        "dev-yummy-join-hist" => self.user_join_hist(consumer, &message).await,
                        _ => {
                            error!(
                                "[Error][MainController->process_stream] Unknown topic: {}",
                                topic
                            );
                            continue;
                        }
    ...
    #[doc = "회원 가입을 위한 이메일 확인 코드를 보내주는 함수"]
    /// # Arguments
    /// * `message` - Kafka 메시지
    ///
    /// # Returns
    /// * Result<(), anyhow::Error>
    async fn user_join_hist<'a>(
        &self,
        ...
        let send_join_form: SendJoinForm = serde_json::from_str(payload.as_str()).map_err(|e| {
            anyhow!(
                "[Error][MainController->user_join_hist] JSON deserialization failed: {:?}",
                e
            )
        })?;

        /* html 템플릿 */
        let html_template: String = fs::read_to_string(Path::new(HTML_JOIN_CHECK_TEMPLATE_PATH.as_str()))?;
        ...
        info!(
            "Successfully sent user JOIN Check information to '{}'.",
            send_join_form.user_email()
        );
    ...
    async fn user_id_hist<'a>(
        ...
        info!(
            "Successfully sent user ID information to '{}'.",
            send_id_form.user_email()
        );
    ...
    async fn user_pw_hist<'a>(
        ...
        info!(
            "Successfully sent the user's temporary password to '{}'",
            send_pw_form.user_email()
        );
